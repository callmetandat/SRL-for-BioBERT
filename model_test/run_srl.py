...
class SrlProcessor(DataProcessor):
    ...
    def get_labels(self):
        return ["[PAD]",'X', 'B-AM-ADV', 'I-A3', 'B-A0', 'O', 'B-AM-NEG', 'B-AR', 'I-AR', 'B-AM-MAN', 'B-A1', 'I-A0', 'I-A4', 'B-A2', 'B-AM-MOD', 'B-A3', 'B-A4', 'I-A2', 'V', 'I-A1', "[CLS]", "[SEP]"] 
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer,mode):
    ...
    for i, word in enumerate(textlist):
        ...
        for m,  tok in enumerate(token):
            #neu nhieu token thi X co nghia la giong label truoc do
            #check lai cho nay coi dat label giong nhau hay dat X
            if m == 0:
    ...
    assert len(predicate) == max_seq_length
...
def file_based_input_fn_builder(input_file, seq_length, is_training, drop_remainder):
    ...
    def _decode_record(record, name_to_features):
        ...
        for name in list(example.keys()):
            ...
            if t.dtype == tf.int64:
                #t = tf.to_int32(t)
                t = tf.cast(tf.constant(1, dtype=tf.int64), tf.int32)
            example[name] = t
        return example
...
def main():
    ...
    if FLAGS.do_predict:
        ...
        if os.path.exists(label2idPath):
            ...
            logging.info("***** Creating label2id.pkl during predict (not recommended) *****")
            logging.info("***** Warning! label2id.pkl not exist *****")
            label2id = {}
            for i, label in enumerate(label_list):
                label2id[label] = i
                id2label = {value:key for key,value in label2id.items()}
        ...
        for file in listfile:
            ...
            filed_based_convert_examples_to_features(predict_examples, label_list,
                                                    ...
                                                    predict_file,mode=name1)
                                
            logging.info("***** Running prediction*****")
            logging.info("  Num examples = %d", len(predict_examples))
            logging.info("  Batch size = %d", FLAGS.predict_batch_size)
            logging.info("  Example of predict_examples = %s", predict_examples[0].text)
            if FLAGS.use_tpu:
            ...
            logging.info("***** token-level evaluation results *****")
            for key in sorted(prf.keys()):
                logging.info("  %s = %s", key, str(prf[key]))
